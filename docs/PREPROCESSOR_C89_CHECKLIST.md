...
- [~] Self-referential object-like macro termination (subset: `#define A A + 1` expands to `A + 1` without runaway growth) — `tests/test_preprocessor_macro_disable_during_expansion.py`
- [~] Self-referential function-like macro termination (subset: `#define F(x) F(x)+1` expands to `F(x)+1` once) — `tests/test_preprocessor_function_like_hideset_termination.py`
- [~] Mutual recursion termination (subset: `A<->B` and `F<->G` do not crash or grow unbounded) — `tests/test_preprocessor_hideset_mutual_recursion.py`
...
- [~] `##` with empty arguments (subset: empty object-like macros and empty actuals tolerated; see `tests/test_preprocessor_macro_token_paste_empty_args.py`)
...
- [~] Diagnostics with file/line ranges (caret), include stacks (subset: unified `file:line:` prefix for most errors incl. #if/#elif expression failures + include stack when applicable; caret ranges still TODO) — `tests/test_preprocessor_diagnostics_locations.py`, `tests/test_preprocessor_diagnostics_unified_L.py`
