...
- **PARTIAL** storage class (`static`/`extern`/`auto`/`register` subset + `&register` rejected + `extern` initializer rejected) — `tests/test_storage_class.py`, `tests/test_auto_register.py`, `tests/test_extern_initializer.py`
