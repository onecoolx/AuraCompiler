...
- **PARTIAL** Preprocessor macro operators (subset): `#` (stringize) and `##` (token paste, incl. best-effort rescan of pasted result; args not expanded pre-paste) — `tests/test_preprocessor_macro_operators.py`, `tests/test_preprocessor_macro_token_paste_rescan.py`, `tests/test_preprocessor_macro_token_paste_args_not_expanded.py`
