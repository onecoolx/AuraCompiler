...
- **PARTIAL** storage class (`static`/`extern`/`auto`/`register` subset + `&register` rejected + `extern` initializer rejected) — `tests/test_storage_class.py`, `tests/test_auto_register.py`, `tests/test_extern_initializer.py`, `tests/test_local_static.py`
...
- **PARTIAL** integer promotions: sign/zero extension for `char`/`short` and basic comparisons (tests) — `tests/test_integer_promotions_more.py`
  ...
  - [x] signed `char` promotion applies in `*` (subset) — `tests/test_int_conversions_signed_char_mul.py`
...
- **PARTIAL** pointer arithmetic completeness — `tests/test_pointer_arith.py` (more TODO)
  ...
  - [x] `char*` pointer +/- and `+=` scale by 1 (subset) — `tests/test_pointer_arith_char_scaling.py`
...
- **PARTIAL** switch/case/default (subset; duplicate `case`/multiple `default` rejected)
  ...
  - [x] no match + no default executes nothing — `tests/test_switch_no_match_returns_0.py`
  - [x] reject duplicate `case` labels — `tests/test_switch_semantics.py`
  - [x] reject multiple `default` labels — `tests/test_switch_semantics.py`
...
- **PARTIAL** Preprocessor `#if` multi-character constants (implementation-defined subset): big-endian byte packing — `tests/test_preprocessor_if_char_literals_multichar.py`
...
- (tests) `#pragma once` `..` path equivalence (subset) — `tests/test_preprocessor_pragma_once_dotdot.py`
