...
> 评估日期：2026-04-27
> 基线：1825 pycc tests passing，cJSON 1.7.19 编译通过，sqlite3 parser 100%
...
## 任务 1：统一类型大小计算为 TargetInfo 抽象

**现状**：
- `_type_size()` 在 ir.py 定义，被 ir.py (~15处)、semantics.py (~3处) 调用
- codegen.py 有独立的 `_type_size_bytes()`
- semantics.py 有独立的 `size_align()`
- 三套硬编码的类型大小逻辑，值相同但代码重复

**工作量**：新建 `TargetInfo` 类，统一三处实现，修改 ~40 个调用点

**复杂度**：中等偏高 | **预估**：3-4 小时 | **依赖**：无

---

## 任务 2：统一局部初始化器降低为递归类型驱动函数
...
**复杂度**：高 | **预估**：6-8 小时 | **依赖**：任务 2

---

## 任务 4：IR 架构重构（HIR → LIR → Assembly）
...
- 这是根本性架构重写，不是单个任务而是一个项目

**工作量**：需 3-5 个独立 spec，每个 spec 都是中等规模重构

**复杂度**：极高 | **预估**：40-60 小时（分 5 阶段，每阶段 8-12h）| **依赖**：任务 1
...
**复杂度**：中等 | **预估**：4-6 小时（假设任务 4 已完成）| **依赖**：任务 4
